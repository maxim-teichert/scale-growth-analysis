...
E1_vs_covar2 <- ggplot(aes(x = temp, y = E1), data = errors) +
  geom_point() +
  geom_smooth(method = "loess") +
  geom_hline(yintercept = 0, lty = 2) +
  labs(x = "temperature (average)", y = "residuals")

E1_vs_covar3 <- ggplot(aes(x = phos, y = E1), data = errors) +
  geom_point() +
  geom_smooth(method = "loess") +
  geom_hline(yintercept = 0, lty = 2) +
  labs(x = "phosphorus (average)", y = "residuals")
...
E1_vs_F1 + E1_vs_covar1 + E1_vs_covar2 + E1_vs_covar3 + E1_vs_covar4 + qq
...
row.names(tbl_org)[row.names(tbl_org) == "temp"] <- "temperature"
row.names(tbl_org)[row.names(tbl_org) == "phos"] <- "phosphorus"
row.names(tbl_org)[row.names(tbl_org) == "age_class"] <- "age class"
row.names(tbl_org)[row.names(tbl_org) == "temp:age_class"] <- "temperature*age class"
row.names(tbl_org)[row.names(tbl_org) == "phos:age_class "] <- "phosphorus*age class"
...
#temperature
emm_temp <- emtrends(mm,
                specs = pairwise ~ "age_class",
                var = "temp",
                infer = TRUE)

emm_temp_trends <- data.frame(emm_temp$emtrends) %>% 
  rename("age class" = age_class,
         coefficient = temp.trend) %>% 
  mutate(coefficient = round(coefficient, 2),
         SE = round(SE, 2),
         lower.CL = round(lower.CL, 2),
         upper.CL = round(upper.CL, 2),
